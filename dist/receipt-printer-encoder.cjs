...
var receipt_printer_encoder_exports = {};
__export(receipt_printer_encoder_exports, {
  default: () => receipt_printer_encoder_default
});
module.exports = __toCommonJS(receipt_printer_encoder_exports);
var import_canvas_dither = __toESM(require("canvas-dither"), 1);
var import_canvas_flatten = __toESM(require("canvas-flatten"), 1);
var import_codepage_encoder3 = __toESM(require("@point-of-sale/codepage-encoder"), 1);
var import_image_data = __toESM(require("@canvas/image-data"), 1);
var import_resize_image_data = __toESM(require("resize-image-data"), 1);

// src/languages/esc-pos.ts
var import_codepage_encoder = __toESM(require("@point-of-sale/codepage-encoder"), 1);
var LanguageEscPos = class {
  ...
  barcode(value, symbology, options) {
    ...
    const bytes = import_codepage_encoder.default.encode(value, "ascii");
  ...
  qrcode(value, options) {
    ...
    const bytes = import_codepage_encoder.default.encode(value, "iso8859-1");
...
var import_codepage_encoder2 = __toESM(require("@point-of-sale/codepage-encoder"), 1);
var LanguageStarPrnt = class {
  ...
  barcode(value, symbology, options) {
    ...
    const bytes = import_codepage_encoder2.default.encode(value, "ascii");
    const identifier = typeof symbology === "string" ? symbologies[symbology] : symbology;
  ...
  qrcode(value, options) {
    ...
    const bytes = import_codepage_encoder2.default.encode(value, "iso8859-1");
...
var _ReceiptPrinterEncoder = class _ReceiptPrinterEncoder {
  ...
  codepage(codepage) {
    ...
    if (!import_codepage_encoder3.default.supports(codepage)) {
  ...
  image(input, width, height, algorithm, threshold) {
    ...
    if (type == "node-read-image") {
      image = new import_image_data.default(input.width, input.height);
      image.data.set(input.frames[0].data);
    }
    if (type == "node-sharp") {
      image = new import_image_data.default(input.info.width, input.info.height);
      image.data.set(input.data);
    }
    if (type == "ndarray") {
      image = new import_image_data.default(input.shape[0], input.shape[1]);
      image.data.set(input.data);
    }
    if (type == "object") {
      image = new import_image_data.default(input.width, input.height);
    ...
    if (width !== image.width || height !== image.height) {
      image = (0, import_resize_image_data.default)(image, width, height, "bilinear-interpolation");
    }
    if (width !== image.width || height !== image.height) {
      throw new Error("Image could not be resized");
    }
    image = import_canvas_flatten.default.flatten(image, [255, 255, 255]);
    switch (algorithm) {
      case "threshold":
        image = import_canvas_dither.default.threshold(image, threshold);
        break;
      case "bayer":
        image = import_canvas_dither.default.bayer(image, threshold);
        break;
      case "floydsteinberg":
        image = import_canvas_dither.default.floydsteinberg(image);
        break;
      case "atkinson":
        image = import_canvas_dither.default.atkinson(image);
        break;
    }
    __privateGet(this, _composer).flush({ forceFlush: true, ignoreAlignment: true });
    if (__privateGet(this, _composer).align !== "left") {
...
encodeText_fn = function(value, codepage) {
  if (codepage !== "auto") {
    const fragment = import_codepage_encoder3.default.encode(value, codepage);
  ...
  const fragments = import_codepage_encoder3.default.autoEncode(value, __privateGet(this, _codepageCandidates));
  const buffer = [];
